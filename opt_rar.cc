...
/*bool dc_impl(map<int, CircuitNode> &orNodeList, map<int, int> ns, map<int, int> nd)
{
    map<int, int>::iterator itrmi;
    itrmi = ns.begin();
    int ns_line = itrmi->first;
    int ns_value = itrmi->second;
    itrmi = nd.begin();
    int nd_line = itrmi->first;
    int nd_value = itrmi->second;
    
    
}*/


void rar(map<int, CircuitNode> &masterNodeList, map<int, CircuitNode> &orNodeList, map<int, CircuitNode> &brNodeList, vector<FaultList> &orFaultList, multimap<int, FaultList> &cost_sf, int &first_cost, vector<string> &testPool)
{
    redFILE1 << endl<<"$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$"<<endl;
   ...
   // print_circuit(orNodeList, redFILE1);
	...
	//	compute_cost(orNodeList, orFaultList, cost_sf, first_cost);	
        ...
        rar(masterNodeList, orNodeList, brNodeList, orFaultList, cost_sf, first_cost, testPool);
	...
	else
	{
	    ...
	    for(itrmi = MA_act.begin(); itrmi != MA_act.end(); itrmi++)
	    {
	        ...
	        if(itrm->second.nodeType == 2)
	        {
	            ...
	            itrm2 = orNodeList.find((*itrv)->lineNumber);
                  ...
                  //  redFILE1 << "2. ns = "<<ns<<endl;                   
	                ...
	                for(itrmi1 = MA_forced.begin(); itrmi1 != MA_forced.end(); itrmi1++)
	                {
	                    ...
	                    {   
                        	...
                        	if(itrm1->second.nodeType == 2) //branch
                        	{
                        	    itrv = itrm1->second.pointFanIn.begin();
                        	    itrm2 = masterNodeList.find((*itrv)->lineNumber);
                        	    if(itrm2->second.nodeType == 1)
                        	       nd = itrm2->second.lineNumber;
                        	}
   ...
   if(!cand_set.empty())
   {
    	...
    	{    	   
    	    ...
    	    int outputLevel = SetLineLevel(tmp_masterNodeList);
	        vector<int> *levelSet;
	        levelSet = new vector<int>[outputLevel];
	        int level;
            for(itrm = tmp_masterNodeList.begin(); itrm != tmp_masterNodeList.end(); itrm++)
            {
    	    ...
    	    if(testPool.size() > 0)
            {
                ...
                vector<int> correct_output = parallel_test_simulation(tmp_masterNodeList, fakeFault, testPool, outputLevel, levelSet);
                vector<int> test_output = parallel_test_simulation(tmp_masterNodeList, objFault, testPool, outputLevel, levelSet);
    	    ...
    	    if(isTestable_wa_ff == 0)    	        
            {
        	    ...
        	    isTestable_wa_podem = podem_sim(tmp_orNodeList, tmp_masterNodeList, objFault, test, outputLevel, levelSet);
        	    delete []levelSet;
        	    ftime(&endTime2);
        	    double runtime_p = ((endTime2.time - startTime2.time)*1000 + (endTime2.millitm - startTime2.millitm))/1000.0;
        	    redFILE1 << "runtime_p = "<<runtime_p<<endl;
        	    runtime_podem += runtime_p;
        	    ...
        	    {    	        
                	...
                	map<int, CircuitNode> bak_masterNodeList;
                	merge_circuit(bak_masterNodeList, tmp_orNodeList, brNodeList);	  
                	multimap<int, FaultList> untFaultSet;             	    	    	
        	        idtf_unt(tmp_orNodeList, bak_masterNodeList, ICA, untFaultSet);
                    ...
                    if(newly_unt_faults.size() == 1)
                    {
                        ...
                        redFILE1 << "1 run logic simulation"<<endl;
                        run_logic_simulation(tmp_masterNodeList, totalInputs, masterInputVector);
                    }
                    else 
                    {
                        ...
                        for(itrmf_r1 = newly_unt_faults.rbegin(); itrmf_r1 != newly_unt_faults.rend(); itrmf_r1++, k++)
                        {
                            ...
                            if(k == 0)
                            {
                                ...
                                redFILE1 << "2 run logic simulation"<<endl;
                        run_logic_simulation(tmp_masterNodeList_new, totalInputs, masterInputVector);
                                continue;
                            } 
                            itrm = tmp_orNodeList.find(nsa_line);
                            if(itrm == tmp_orNodeList.end())
                                continue;                            
                            int outputLevel = SetLineLevel(tmp_masterNodeList_new);
	                        vector<int> *levelSet;
	                        levelSet = new vector<int>[outputLevel];
	                        int level;
                            for(itrm = tmp_masterNodeList_new.begin(); itrm != tmp_masterNodeList_new.end(); itrm++)
                            {
                                level = itrm->second.lineLevel;
                                if(level > 0)
                                    levelSet[level-1].push_back(itrm->second.lineNumber);
                    	    ...
                    	    int isTestable = podem_sim(tmp_orNodeList, tmp_masterNodeList_new, itrmf_r1->second, test, outputLevel, levelSet);
                    	    delete []levelSet;
                            if(!isTestable)
                            {
                                simplify_circuit(tmp_orNodeList, nsa_line, nsa_value);
                                redFILE1 << "after remove redundancy, circuit size = "<<tmp_orNodeList.size()<<endl;
                                merge_circuit(tmp_masterNodeList_new, tmp_orNodeList, brNodeList);
                                redFILE1 << "3 run logic simulation"<<endl;
                        run_logic_simulation(tmp_masterNodeList_new, totalInputs, masterInputVector);
                            } 
                            else
            ...
            if((isTestable_wa_ff == 1 || isTestable_wa_podem == 1) && this_cost > 6)
            {
                ...
                if(isTestable == 1)
                    redFILE1 << "wrong: this fault is testable1"<<endl;
                ...
                for(itrmi = MA_sa.begin(); itrmi != MA_sa.end(); itrmi++)
                {
                    ...
                    if(itrmi1 != MA_wa.end())
                    { 
                        itrm1 = tmp_masterNodeList.find(itrmi1->first);
                        if(itrm1->second.circuitIndex != 1)
                            continue;
                ...
                else 
                {
                    ...
                    for(itrmi = ndp_set.begin(); itrmi != ndp_set.end(); itrmi++)
                    {
                        ...
                        merge_circuit(tmp_masterNodeList, tmp_orNodeList, brNodeList);
                        redFILE1 << "after remove redundancy, circuit size = "<<tmp_orNodeList.size()<<endl;
                        redFILE1 << "4 run logic simulation"<<endl;
                        run_logic_simulation(tmp_masterNodeList, totalInputs, masterInputVector);
...
void opt_rar(map<int, CircuitNode> &orNodeList, map<int, CircuitNode> &brNodeList, map<int, CircuitNode> &masterNodeList, vector<FaultList> &orFaultList)
{
