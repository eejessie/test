...
void run_logic_simulation_cut(map<int, CircuitNode> &orNodeList, int totalInputs, vector<string> & InputVector, map<int, int> &truthTable)
{
	int	flag = 0;
	...
	else */
	{
	    ...
	    {		
		 ...
		 //   LogicSimulation(orNodeList, totalInputs, InputVector[j], outputLevel, levelSet, res);
	        ...
	        {
	            itrm = orNodeList.find(cut_nodes[i]);
